   ...
   public function showCheckout(){
        if(!Auth::check()){
            ...
            for ($i=0; $i < sizeof($checkout['checkout_items']); $i++) { 
                ...
                $checkout['shipping_product'] += $checkout["ci_quantity"][$i] * $checkout['checkout_items'][$i]['product_dc'];
                if ($checkout['checkout_items'][$i]['pay_on_delivery'] == 1) {
                    $checkout['payment_type'] = 0;
                }
            }
        ...
        switch ($request->checkout_action) {
            ...
            case 'process_checkout':
                if(!Auth::check()){
                    ...
                    if($request->payment_type == "before_delivery"){
                        $checkout['payment_type'] = 0;
                    }
                    ...
                    if (isset($checkout_sf_object['shipping_fare']['sf_town']) && $checkout_sf_object['shipping_fare']['sf_town'] == 'Accra') {
                    ...
                    for ($i=0; $i < sizeof($checkout['checkout_items']); $i++) {
                        ...
                        $order_item->oi_type                = $checkout["checkout_items"][$i]["pay_on_delivery"];
