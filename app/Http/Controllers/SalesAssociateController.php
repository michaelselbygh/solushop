...
class SalesAssociateController extends Controller
{
    ...
    public function processAddOrder($customerID, $addressID, Request $request){
        ...
        for ($i=0; $i < sizeof($order_item_skus); $i++) { 
            $oi = StockKeepingUnit::where('id', $order_item_skus[$i])->with("product")->first()->toArray();
            if ($oi['product']['pay_on_delivery'] == 1) {
                $payment_type = 0;
            }
        }
        ...
        if (isset($address['shipping_fare']['sf_town']) && $address['shipping_fare']['sf_town'] == 'Accra') {
            $order->payment_type = $payment_type;
        } elseif(isset($address['shipping_fare']['sf_town']) && $address['shipping_fare']['sf_town'] == 'Tema'){
            $order->payment_type = $payment_type;
        }
        elseif(isset($address['shipping_fare']['sf_town']) && $address['shipping_fare']['sf_town'] == 'Weija'){
            $order->payment_type = $payment_type;
        }else {
        ...
        if ($order->payment_type == 1) {
            /*--- Notify customer ---*/
            $sms_message = "Hi ".$customer->first_name.", kindly pay GHS ".($shipping + (0.99 * $order_item_subtotal))." to our dispatch on delivery for your order $order_id.";
            ...
            $data = array(
                'subject' => 'Order Initiated - Solushop Ghana',
                'name' => $customer->first_name,
                'message' => "Kindly pay GHS ".($shipping + (0.99 * $order_item_subtotal))." to our dispatch on delivery for your order $order_id."
            );
